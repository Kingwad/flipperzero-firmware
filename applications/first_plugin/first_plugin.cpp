...
bool FirstPlugin::Init() {
	bool success = false;
	if (m_initialized) {
		FURI_LOG_E("First_plugin", "Double initialization\r\n");
	} else {
		m_initialized = true;
...
int32_t FirstPlugin::Run(void* _args) {
	...
	for(bool processing = true; processing;) {
		...
		if (eventStatus == osOK) {
			...
			FURI_LOG_D("First_plugin", "Run: osMessageQueue: event timeout");
		}
...
void FirstPlugin::RenderCallback(Canvas* _canvas, void* _context) {
	ValueMutex* stateMutex = static_cast<ValueMutex*>(_context);
...
void FirstPlugin::InputCallback(InputEvent* _event, void* _context) {
	osMessageQueueId_t eventQueue = static_cast<osMessageQueueId_t>(_context);
	PluginEvent event;
